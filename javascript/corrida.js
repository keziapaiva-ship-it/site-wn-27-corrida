...
// ▶️ iniciar
gerarRota();
// Apenas adiciona e remove a classe visual
function selecionarVeiculo(elementoClicado) {
    // 1. Remove a marcação de todos os carros
    const todasOpcoes = document.querySelectorAll('.vehicle-option');
    todasOpcoes.forEach(opcao => {
        opcao.classList.remove('selected');
    });

    // 2. Coloca a marcação apenas no que foi clicado
